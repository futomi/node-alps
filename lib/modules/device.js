	...
	let promise = new Promise((resolve, reject) => {
		...
		p.discoverAllServicesAndCharacteristics((error, service_list, char_list) => {
			...
			if (!(this._chars['custom1'] && this._chars['custom2'] && this._chars['custom3'])) {
				...
				reject(new Error('The device seems not to be a ALPS Sensor Network Module.'));
				return;
			}
			if (timer) {
				clearTimeout(timer);
...
* Method: isConnected()
* ---------------------------------------------------------------- */
AlpsDevice.prototype.isConnected = function () {
...
* Method: getStatus()
* ---------------------------------------------------------------- */
AlpsDevice.prototype.getStatus = function () {
	let promise = new Promise((resolve, reject) => {
		this.write(0x2E).then((data) => {
			resolve(data);
		}).catch((error) => {
			reject(error);
		});
...
* ---------------------------------------------------------------- */
AlpsDevice.prototype.getDeviceName = function () {
	let promise = new Promise((resolve, reject) => {
		this.read(0x15).then((data) => {
			resolve(data);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: setDeviceName(p)
* - p = {
*     deviceName: 'SNM00' // 1 to 18 characters (a-zA-Z0-9)
*   }
* ---------------------------------------------------------------- */
AlpsDevice.prototype.setDeviceName = function (p) {
	let promise = new Promise((resolve, reject) => {
		if (!p || typeof (p) !== 'object') {
			reject(new Error('The parameter is required.'));
			return;
		}
		if(!('deviceName' in p)) {
			reject(new Error('The `deviceName` is required.'));
			return;
		}
		let name = p['deviceName'];
		if(typeof(name) !== 'string') {
			reject(new Error('The `deviceName` must be a string.'));
			return;
		}
		if(!/^[a-zA-Z0-9]{1,18}$/.test(name)) {
			reject(new Error('The `deviceName` must be a string (1 to 18 characters [a-zA-Z0-9]).'));
			return;
		}
		this.write(0x15, {deviceName: name}).then((data) => {
			resolve();
		}).catch((error) => {
			reject(error);
		});
